...
WANDB_API_KEY=$wandb_key accelerate launch --config_file accelerate_config/fsdp_8gpu.yaml --main_process_port 29500 launch.py \
    ...
    model=$model \
    datasets=[$dataset] \
    exp_name=sympo_$loss_type/noise_ratio_$noise_ratio/seed_$seed \
    seed=$seed \
...
python -m train.sample \
    .cache/data/$dataset/$model_name/sympo_$loss_type/noise_ratio_$noise_ratio/seed_$seed/FINAL \
    --gpu_count 2 \
    --output_file .cache/outputs/$dataset/$model_name/sympo_$loss_type/noise_ratio_$noise_ratio/seed_$seed/completions.json \
    --datasets $dataset \
    --mode custom_dataset \
    --max_tokens 512 \

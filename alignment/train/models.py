...
class ReferenceModelWrapper(nn.Module):
    ...
    def __init__(
        self, reference_accelerator, reference_model, tokenizer, config, iterators
    ):
        """
        Args:
        ...
        self.reference_model, self.tokenizer, self.iterators = (
            reference_accelerator.prepare(
                self.reference_model, self.tokenizer, self.iterators
            )
        )
    ...
    def _precompute_log_probs(self):
        ...
        pbar = tqdm(
            disable=not self.reference_accelerator.is_local_main_process,
            dynamic_ncols=True,
        )
        ...
        with torch.no_grad():
            for data_iterator in self.iterators:
                for batch in data_iterator:
                    batch = {
                        k: (
                            v.to(self.reference_accelerator.device)
                    ...
                    prefixes = [
                        k[: k.index("_")]
                        for k in batch
                        if k.endswith("_combined_input_ids")
                    ]
                    ...
                    for prefix in prefixes:
                        ...
                        batch_logprobs = self._compute_log_probs(
                            logits, batch[f"{prefix}_labels"]
                        ).tolist()

                        for k, v in zip(
                            batch[f"{prefix}_combined_input_ids"].tolist(),
                            batch_logprobs,
                        ):
        ...
        if self.reference_accelerator.is_main_process:
            with open(
                os.path.join(
                    self.config.local_run_dir, "cached_reference_logprobs.pkl"
                ),
                "wb",
            ) as f:
    ...
    def forward(self, input_ids: Dict[str, Any], *args, **kwargs) -> torch.Tensor:
        ...
        """
        batch_logprobs = [
            torch.Tensor(self.logprobs[tuple(self._remove_padding(k))])

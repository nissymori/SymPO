...
def get_base_model_state_dict_from_peft(peft_state_dict, lora_alpha, lora_r):
    ...
    for name in peft_state_dict.keys():
        if "lora_A" in name:
            ...
            new_name = name.replace("lora_A.default.", "").replace(
                "base_model.model.", ""
            )
            state_dict[new_name] = (
                peft_state_dict[base_param_name] + (lora_b @ lora_a) * scaling
            )
...
def entropy_from_logits(logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    entropy = masked_mean(
        torch.logsumexp(logits, axis=-1) - torch.sum(pd * logits, axis=-1), mask
    )
    return entropy

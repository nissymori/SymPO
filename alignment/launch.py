...
from train import dataloader, models, trainers
...
def main(config: DictConfig):
    ...
    if accelerator.state.fsdp_plugin is not None:
        accelerator.state.fsdp_plugin.transformer_layer_cls_to_wrap = (
            config.model.block_name
        )

    # Calculate microbatch sizes
    if config.model.batch_size % accelerator.num_processes == 0:
        config.model.microbatch_size = (
            config.model.batch_size / accelerator.num_processes
        )
    else:
        raise ValueError(
            f"{config.model.batch_size} needs to be divisible by the number of processes"
        )

    if config.model.eval_batch_size % accelerator.num_processes == 0:
        config.model.eval_microbatch_size = (
            config.model.eval_batch_size / accelerator.num_processes
        )
    else:
        raise ValueError(
            f"{config.model.eval_batch_size} needs to be divisible by the number of processes"
        )
    ...
    if TrainerClass.use_reference_model:
        ...
        reference_kwargs = {
            ...
            "attn_implementation": (
                ...
                if config.model.policy_dtype in ["float16", "bfloat16"]
                else "eager"
            ),
        }
        ...
        reference_model = reference_cls.from_pretrained(
            reference_path, **reference_kwargs
        )
        ...
        if config.cache_reference_logprobs:
            ...
            if reference_accelerator.state.fsdp_plugin is not None:
                reference_accelerator.state.fsdp_plugin.transformer_layer_cls_to_wrap = (
                    config.model.block_name
                )
    ...
    optimizer = getattr(torch.optim, config.optimizer)(
        policy.parameters(), lr=config.lr
    )
    ...
    main_scheduler = CosineAnnealingLR(
        optimizer,
        T_max=train_iterator.num_training_steps - config.warmup_steps,
        eta_min=0,
    )
    ...
    if config.model.from_checkpoint:
        ...
        optimizer_state.update(
            torch.load(
                os.path.join(config.model.from_checkpoint, "optimizer.pt"),
                map_location="cpu",
            )
        )
        ...
        scheduler_state = torch.load(
            os.path.join(config.model.from_checkpoint, "scheduler.pt")
        )
        scheduler.load_state_dict(scheduler_state)

        metrics = json.load(
            open(os.path.join(config.model.from_checkpoint, "metrics.json"))
        )
    ...
    if config.model.reward_model.path:
        ...
        reward_kwargs = {
            ...
            "attn_implementation": (
                config.model.reward_model.attn_implementation
                if config.model.reward_model.dtype in ["float16", "bfloat16"]
                else "eager"
            ),
        }
        reward_model = reward_hf_model_class.from_pretrained(
            config.model.reward_model.path, **reward_kwargs
        )
    else:
    ...
    if config.loss.trainer == "sympo":
        assert (
            config.loss.loss_type == trainer.config.loss.loss_type
        )  # assert that the loss type is the same as the one specified in the config
